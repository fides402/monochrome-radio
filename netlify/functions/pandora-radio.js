/**
 * Pandora radio proxy — pure-JS Blowfish (no OpenSSL dependency).
 * Works on Node 16, 18, 20+ regardless of OpenSSL version.
 */

const axios = require('axios');

// ── Pure-JS Blowfish-ECB ──────────────────────────────────────────────────────
// P-array and S-boxes initialised from pi digits (standard Blowfish)
const ORIG_P = [
...
class Blowfish {
  constructor(key) {
    this.P = ORIG_P.slice();
    this.S = ORIG_S.map(s => s.slice());
    const keyBytes = Buffer.isBuffer(key) ? key : Buffer.from(key, 'utf8');
    let j = 0;
    for (let i = 0; i < 18; i++) {
      let data = 0;
      for (let k = 0; k < 4; k++) {
        data = uint32((data << 8) | keyBytes[j % keyBytes.length]);
        j++;
      }
      this.P[i] = uint32(this.P[i] ^ data);
    }
    let l = 0, r = 0;
  ...
  _F(x) {
    const a = (x >>> 24) & 0xff, b = (x >>> 16) & 0xff,
          c = (x >>> 8)  & 0xff, d =  x         & 0xff;
    return uint32((uint32(this.S[0][a] + this.S[1][b]) ^ this.S[2][c]) + this.S[3][d]);
  }
  _encryptBlock(l, r) {
    for (let i = 0; i < 16; i++) {
      l = uint32(l ^ this.P[i]);
      r = uint32(r ^ this._F(l));
      [l, r] = [r, l];
    }
    [l, r] = [r, l];
    r = uint32(r ^ this.P[16]);
    l = uint32(l ^ this.P[17]);
    return [l, r];
  }
  _decryptBlock(l, r) {
    for (let i = 17; i > 1; i--) {
  ...
  encrypt(buf) {
    ...
    for (let i = 0; i < buf.length; i += 8) {
      let l = buf.readUInt32BE(i), r = buf.readUInt32BE(i + 4);
      [l, r] = this._encryptBlock(l, r);
      out.writeUInt32BE(l, i); out.writeUInt32BE(r, i + 4);
    }
    return out;
  }
  decrypt(buf) {
...
function bfDecrypt(hexStr, key) {
  ...
  const buf = bf.decrypt(Buffer.from(hexStr, 'hex'));
  return buf.toString('utf8').replace(/\0/g, '');
}

function bfEncrypt(str, key) {
  const pad    = (8 - (str.length % 8)) % 8;
  const padded = Buffer.from(str + '\0'.repeat(pad), 'utf8');
  return new Blowfish(key).encrypt(padded).toString('hex');
}

// ── Partner credentials (android client) ─────────────────────────────────────
const PARTNER = {
  username:   'android',
...
async function createSession(email, password) {
  const partner = await pandoraPost('auth.partnerLogin', {
    ...
    deviceModel: PARTNER.device, version: PARTNER.version, includeUrls: true,
  });
  ...
  const user = await pandoraPost('auth.userLogin',
    { loginType: 'user', username: email, password, partnerAuthToken: partner.partnerAuthToken,
      syncTime: Math.floor(Date.now() / 1000) - timeOffset },
    { partner_id: partner.partnerId, auth_token: partner.partnerAuthToken },
    true,
  );
...
const authParams = s => ({ partner_id: s.partnerId, user_id: s.userId, auth_token: s.userToken });
...
exports.handler = async (event) => {
  ...
  const email    = process.env.PANDORA_EMAIL    || 'francescocapuani92@gmail.com';
  const password = process.env.PANDORA_PASSWORD || 'fides9219';
  ...
  try {
    ...
    const search = await pandoraPost('music.search',
      { searchText: `${artist} ${title}`, includeNearMatches: true, includeGenreStations: false,
        userAuthToken: session.userToken, syncTime: syncNow(session) },
      params, true,
    );

    const song = (search.songs || [])[0];
    if (!song) return { statusCode: 404, headers: CORS, body: JSON.stringify({ error: 'Track not found on Pandora' }) };
    ...
    return { statusCode: 200, headers: CORS, body: JSON.stringify({ stationName: station.stationName, tracks: unique }) };
  } catch (e) {
